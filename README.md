...
3. The `ManagedServiceAccount` secrets should be backed up on the hub but they don't have the `cluster.open-cluster-management.io/backup` label on creation. For this reason, the backup controller looks regularly for `ManagedServiceAccount` secrets under the managed clusters namespaces, and adds the backup label if not found. It is possible that a `ManagedServiceAccount` secret may not be included in a backup if the backup schedule runs before this backup label is set on the `ManagedServiceAccount` secret. 
...
5. If the token is valid when the restore operation is executed but the managed cluster is not accessible at the time the restore is executed, the auto import operation fails and will not retry to reconnect. In this case the failure should be reported by the auto-import component logs.
